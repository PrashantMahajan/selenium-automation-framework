...
import com.scholastic.framework.context.SeleniumContext;
...
public class ExcelsheetRead extends ExcelFunc {
	...
	public void startFunction() {
		...
		try {
			v_objWorkbook = SeleniumContext.getInstance().getWorkbook(this.g_sFileName);
			try {
				if (null == v_objWorkbook) {
					v_objWorkbook = new XSSFWorkbook(ClassLoader.getSystemResourceAsStream(this.g_sFileName));
					SeleniumContext.getInstance().addWorkbook(this.g_sFileName, v_objWorkbook);
				}
